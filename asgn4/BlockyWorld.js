...
const VSHADER_SOURCE = `
  ...
  void main() {
    ...
    v_UV       = a_UV;
    v_Normal   = a_Normal;
    v_NormalDir = u_NormalMatrix * a_Normal;
    v_LightDir  = u_lightPos - worldPos.xyz;
    v_ViewDir   = u_eyePos   - worldPos.xyz;
  }
`;
...
const FSHADER_SOURCE = `
  ...
  uniform vec3  u_spotDir;      // world-space direction the spotlight points
  uniform float u_spotCutoff;   // cos of the outer half-angle of the cone
  void main() {
    if (u_showNormals == 1) {
      gl_FragColor = vec4(normalize(v_NormalDir), 1.0);
      return;
    }
    // u_whichTexture == -3: emissive/unlit (light cube, stars, sun, moon)
    if (u_whichTexture == -3) {
    ...
    vec4 baseColor;
    if      (u_whichTexture == -2) { baseColor = v_Color; }
    else if (u_whichTexture == -1) { baseColor = vec4(v_UV, 1.0, 1.0); }
    else if (u_whichTexture ==  0) { baseColor = texture2D(u_Sampler0, v_UV) * vec4(v_Color.rgb, 1.0); }
    else if (u_whichTexture ==  1) { baseColor = texture2D(u_Sampler1, v_UV) * vec4(v_Color.rgb, 1.0); }
    else                           { baseColor = v_Color; }
    ...
    // Phong coefficients (hard-coded; only diffuse colour comes from JS)
    const float Ka        = 0.3;
    const float Kd        = 1.0;
    const float Ks        = 0.6;
    const float shininess = 32.0;

    // Spotlight attenuation: 1.0 everywhere when off, cone falloff when on.
    // toFrag is the direction FROM the light TO this fragment.
    // smoothstep gives a soft edge between the outer cutoff and a slightly
    // tighter inner ring (cutoff + 0.07 in cosine space ≈ ~4° of softness).
    float spotAtten = 1.0;
    if (u_spotOn == 1) {
      vec3  toFrag   = normalize(-v_LightDir);
      float cosTheta = dot(toFrag, normalize(u_spotDir));
      spotAtten = smoothstep(u_spotCutoff, u_spotCutoff + 0.07, cosTheta);
    }

    vec3 ambient  = Ka * baseColor.rgb;
    vec3 diffuse  = Kd * max(0.0, dot(N, L)) * u_lightColor * baseColor.rgb * spotAtten;
    vec3 specular = Ks * pow(max(0.0, dot(R, V)), shininess) * u_lightColor * spotAtten;
...
let g_showNormals = 0;
let u_lightPos;
let g_lightPos      = [22, 10, 16];  // world-space position of the point light (over master sword)
let g_lightXManual  = false;        // true once the X slider has been moved
let g_lightPaused   = false;        // true when light rotation is paused
let u_NormalMatrix;
let u_lightColor;
let u_eyePos;
let g_lightColor = [1.0, 1.0, 1.0];  // RGB, controlled by color picker
let u_lightOn;
let g_lightOn = 1;
let u_spotOn;
let u_spotDir;
let u_spotCutoff;
let g_spotOn      = 0;
let g_spotCutoff  = Math.cos(30 * Math.PI / 180);  // 30° half-angle default

// Pre-allocated helpers for per-draw normal matrix upload — avoids GC in hot path
const _nm4          = new Matrix4();        // temp 4×4 for invert/transpose
const _nm3          = new Float32Array(9);  // mat3 sent to u_NormalMatrix
const _identityMat3 = new Float32Array([1,0,0, 0,1,0, 0,0,1]);
...
function addActionsForHtmlUI() {
  ...
  document.getElementById('normVis').onclick = function() {
    g_showNormals = g_showNormals ? 0 : 1;
    this.textContent = 'Normal Vis: ' + (g_showNormals ? 'ON' : 'OFF');
  };
  ...
  document.getElementById('lightToggle').onclick = function() {
    g_lightOn = g_lightOn ? 0 : 1;
    this.textContent = 'Lighting: ' + (g_lightOn ? 'ON' : 'OFF');
  };

  document.getElementById('spotToggle').onclick = function() {
    g_spotOn = g_spotOn ? 0 : 1;
    this.textContent = 'Spotlight: ' + (g_spotOn ? 'ON' : 'OFF');
  };

  document.getElementById('pauseLight').onclick = function() {
    g_lightPaused = g_lightPaused ? false : true;
    this.textContent = 'Pause Light: ' + (g_lightPaused ? 'ON' : 'OFF');
  };

  document.getElementById('spotAngle').addEventListener('input', function() {
    g_spotCutoff = Math.cos(Number(this.value) * Math.PI / 180);
    document.getElementById('spotAngleValue').textContent = this.value;
  });

  document.getElementById('lightColor').addEventListener('input', function() {
    const h = this.value;
    g_lightColor[0] = parseInt(h.slice(1, 3), 16) / 255;
    g_lightColor[1] = parseInt(h.slice(3, 5), 16) / 255;
    g_lightColor[2] = parseInt(h.slice(5, 7), 16) / 255;
  });
...
function main() {
  ...
  // Load the Master Sword — async, render() is a no-op until it resolves
  g_sword.materialColors = {
    'sword_Blade':        [0.72, 0.84, 1.00, 1.0],  // blue-silver blade
    'sword_HandleGuard':  [0.88, 0.74, 0.28, 1.0],  // golden crossguard
    'sword_HandleString': [0.30, 0.15, 0.10, 1.0],  // dark leather grip
    'sword_GoldenMark':   [0.96, 0.86, 0.18, 1.0],  // bright gold marks
    'sword_Mark':         [0.78, 0.72, 0.95, 1.0],  // triforce / Zelda icon
  };
  g_sword.load('Twilight Princess Master Sword.obj');
...
function tick() {
  ...
  // Orbit light around the sword position; rotation can be paused via g_lightPaused
  const _LR = 8;
  if (!g_lightPaused) {
    if (!g_lightXManual)
      g_lightPos[0] = 22 + _LR * Math.cos(g_seconds * 0.8);
    g_lightPos[2] = 16 + _LR * Math.sin(g_seconds * 0.8);
  }
  ...
  // Update light position display
  document.getElementById('lightPos').textContent = 
    g_lightPos[0].toFixed(1) + ', ' + 
    g_lightPos[1].toFixed(1) + ', ' + 
    g_lightPos[2].toFixed(1);
...
const g_sphere    = new Sphere();
const g_lightCube = new Cube();   // marker drawn at the light position
const g_sword     = new OBJMesh();
...
function drawWireframe(x, y, z) {
  ...
  gl.uniformMatrix3fv(u_NormalMatrix, false, _identityMat3);
...
function renderAllShapes() {
  ...
  gl.uniform3fv(u_eyePos, g_camera.eye.elements);
  ...
  // Small bright cube marks the light source position (emissive — not self-lit)
  g_lightCube.color      = [g_lightColor[0], g_lightColor[1], g_lightColor[2], 1.0];
  g_lightCube.textureNum = -3;
  g_lightCube.matrix.setTranslate(g_lightPos[0], g_lightPos[1], g_lightPos[2]);
  g_lightCube.matrix.scale(0.25, 0.25, 0.25);
  g_lightCube.render();

  drawWater();

  // Sphere floating above the platform center
  g_sphere.color      = [0.8, 0.3, 0.9, 1.0];
  g_sphere.textureNum = -2;
  g_sphere.matrix.setTranslate(16, 6, 16);
  g_sphere.matrix.scale(1.5, 1.5, 1.5);
  g_sphere.render();

  // Master Sword — standing upright on the platform.
  // The mesh lies flat along Z (tip at z≈-3.74, handle at z≈+1.04).
  // Rotating 90° around X maps z→-y so the blade points up.
  // Translating y by 2.04 puts the handle bottom at y≈1 (ground level).
  g_sword.textureNum = -2;
  g_sword.matrix.setTranslate(22, 2.04, 16);
  g_sword.matrix.rotate(90, 1, 0, 0);
  g_sword.render();
